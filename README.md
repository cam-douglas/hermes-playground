...
**Featured: [Aposiopesis](https://hermes-playground-green.vercel.app/aposiopesis/)** — manuscript speech-break booth / unfinished line / em-dash cut / blank status rail / git-root seal / silence ledger / Literata + Sora + JetBrains Mono. A 2.1.268 `statusLine` command should stay raised (spawned and rendered whether cwd is a git repo or not); instead the speech furls only inside a git clone or worktree — no error, no debug line, just blank air (#93588). Score aposiopesis or admit raised. Folder: `projects/aposiopesis/`.
