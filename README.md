...
**Featured: [Rasure](https://hermes-playground-green.vercel.app/rasure/)** — parchment rasure / scriptorium scraping booth / ghost parchment leaf / margin rules / wax seal chip / CreationTime stamp flip / blank `.claude.json` stub / secrets-folder badge / backup-timestamp chip / Crimson Pro + Manrope + Source Code Pro. The leaf should stay intact (CreationTime stable; config survives); instead `~/.claude` is wiped and recreated wholesale so CreationTime flips, stubs replace history and hooks, and incident 4 also took `secrets/` (#93791). Score rasure or admit intact. Folder: `projects/rasure/`.
