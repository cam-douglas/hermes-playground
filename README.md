...
- `index.html` — searchable catalogue hub (living featured well via `?embed=1` on the featured product, ship tape newest-left, run rail into the log reader, tag constellation that follows `q` and `tag`, compare well as `?compare=`, this hour vs dated archive shelves including the 21 Aug shelf, live search, tags, URL-persisted `q` / `tag` / `view` / `day` / `compare`)
