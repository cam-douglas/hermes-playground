...
**Featured: [Stratum](https://hermes-playground-green.vercel.app/stratum/)** — geology / core-sample / bedding-plane / field-stratigraphy booth / Fraunces + Sora + IBM Plex Mono. The project bed should stay shared (CLAUDE.md / auto-memory sealed with its own cache_control so parallel shafts read it); instead the block sits in messages[0] after the system breakpoint with no cache_control of its own, and 0 of 474 warm starts share it (#94417). Score stratum or admit shared. Folder: `projects/stratum/`.

**Listed: [Tmesis](https://hermes-playground-green.vercel.app/tmesis/)** — manuscript / rhetoric / spliced parchment / editorial desk / open folio / advisor ink / slash-command ribbon booth / Cormorant Garamond + Source Sans 3 + IBM Plex Mono. The folio should stay contiguous (defer local_command until the open assistant message has closed); instead a slash command (/effort) typed while advisor is in flight splices local_command records mid-message, breaks parentUuid, and permanently 400s the session (#86198). Score tmesis or admit contiguous. Folder: `projects/tmesis/`. Still listed. Not featured.

**Listed: [Vedette](https://hermes-playground-green.vercel.app/vedette/)** — cavalry vedette / outpost lantern / picket-line / field-desk booth / Bebas Neue + IBM Plex Sans + Share Tech Mono. The column should stay stationed (parent stays until background Tasks complete); instead headless claude -p exits with its own background subagents still running and still reports success / exit 0 (#94392). Score vedette or admit stationed. Folder: `projects/vedette/`. Still listed. Not featured.

**Listed: [Orloj](https://hermes-playground-green.vercel.app/orloj/)** — Prague orloj / astronomical clock / zodiac dial / automaton tower / calendar dial booth / Bodoni Moda + Karla + Source Code Pro. The tower should stay lasting (schema-promised hour honored during active use); instead Monitor schema caps timeout_ms at 3600000 and ignores persistent, then even at the allowed max reports expires in 30m and dies ~30 minutes during an active session (#94393). Score orloj or admit lasting. Folder: `projects/orloj/`. Still listed. Not featured.
...
**Listed: [Vizard](https://hermes-playground-green.vercel.app/vizard/)** — Elizabethan face-mask / half-mask / masque-ball / gilt-edge vizard / velvet ribbon / dressing-table looking-glass booth / Lora + Public Sans + IBM Plex Mono. The looking-glass should stay pledged (session model choice survives lifecycle); instead backgrounding Remote Control mobile resets a running session's model to Opus 4.8 — reopen and the indicator reads Opus 4.8 again, every time (#94398). Score vizard or admit pledged. Folder: `projects/vizard/`. Still listed. Not featured.

**Listed: [Treacle](https://hermes-playground-green.vercel.app/treacle/)** — Victorian confectionery / copper jam kettle / treacle-well / sticky-ladle / wax-paper twist / enamel kitchen scale / molasses pour booth / Cormorant Infant + Nunito Sans + IBM Plex Mono. The kettle should stay brisk (PowerShell starts promptly like Bash); instead every NEW PowerShell tool call on Claude Desktop (Code tab) Windows waits ~153–160s (median 154.1s / 1244 calls) from tool_use until the command starts, the permission dialog appears only after the wait, and repeats return in 2–3s (#94344). Score treacle or admit brisk. Folder: `projects/treacle/`. Still listed. Not featured.

**Listed: [Somnus](https://hermes-playground-green.vercel.app/somnus/)** — Roman sleep / night-nursery / moon-watch / sleep-clinic booth / Fraunces + Figtree + Source Code Pro. The schedule should stay cadence (recurring cloud trigger stays enabled across a sleep miss); instead a Cowork cloud scheduled task with `requires_local_device:true` is permanently disabled (`suspension_reason=device_absent`) after one fire while the bound Mac is asleep (#94415). Score somnus or admit cadence. Folder: `projects/somnus/`. Still listed. Not featured.

**Listed: [Lemure](https://hermes-playground-green.vercel.app/lemure/)** — Roman restless dead / Lemuria festival / salt-bean rite / lararium household-shrine booth / Bodoni Moda + Figtree + Fira Code. The shrine should stay quiet (no orphan minute-ticks; ghosts absent from the dispatcher, or listed and deletable); instead Desktop ScheduledTasks still walks two deleted/legacy ids once a minute (`ohayo-morning-report`, `mercari-daily-sales-check`) at a vanished `~/Claude/Scheduled/.../SKILL.md` path while Routines UI, MCP list, and on-disk registry omit them and delete is rejected as not found (#94410). Score lemure or admit quiet. Folder: `projects/lemure/`. Still listed. Not featured.

**Listed: [Gauntlet](https://hermes-playground-green.vercel.app/gauntlet/)** — medieval tournament gauntlet / iron glove / riveted cuff / tilting-yard / mail sleeve / bare-hand vs armored fist booth / Cinzel + Karla + Space Mono. The lists should stay ungloved (DISABLE_MOUSE=1 honored; zero enables; paste-open); instead attached background sessions (`claude attach`) enable xterm mouse reporting unconditionally, ignoring CLAUDE_CODE_DISABLE_MOUSE and CLAUDE_CODE_DISABLE_MOUSE_CLICKS, while a directly-launched `claude` in the same terminal honors the opt-out (#94029). Score gauntlet or admit ungloved. Folder: `projects/gauntlet/`. Still listed. Not featured.

**Listed: [Lictor](https://hermes-playground-green.vercel.app/lictor/)** — Roman lictor / fasces / magistrate-procession / wax-tablet ledger / iron-rod bundle / curule-chair / torch-lit forum aisle booth / Cormorant Garamond + Source Sans 3 + IBM Plex Mono. The aisle should stay attested (hooks fired in order; Pre then Post rows written; policy gate held); instead the Claude desktop Code-tab model picker changes the session model without dispatching PreModelSwitch or PostModelSwitch, while CLI `/model` on the same machine still dispatches both — silent policy bypass, no error, no log row (#94053). Score lictor or admit attested. Folder: `projects/lictor/`. Still listed. Not featured.

**Listed: [Lychgate](https://hermes-playground-green.vercel.app/lychgate/)** — parish lychgate / coffin-rest / churchyard-porch / lantern / iron-latch / parish-roll booth / Spectral + Figtree + IBM Plex Mono. The porch should stay reaped (process exit observed → panel shows finished + notification); instead a foreground Bash that hits the 600s timeout is moved to the background, the shell later exits (`ps` empty) but the panel + `/tasks` still say Running for 5–7 hours, and a `run_in_background` remote `ssh HOST 'md5sum …'` hangs ~9h56m on open stdin (#94059). Score lychgate or admit reaped. Folder: `projects/lychgate/`. Still listed. Not featured.

**Listed: [Ouster](https://hermes-playground-green.vercel.app/ouster/)** — common-law ouster / Georgian bailiff-desk / tenancy-roll / wax-seal notice / occupied-flat / iron-key / parchment-writ / ink-stained ledger / street-door booth / Instrument Serif + Manrope + Fragment Mono. The roll should stay tenanted (worktree retained while nested inheritor still runs; iron key stays in the lock); instead a subagent launched with isolation worktree starts a background child that inherits the same worktree (`inheritedWorktreePath`, `spawnDepth: 2`), the parent finishes without changes, Claude Code auto-removes the worktree (`worktreeCleanlyRemoved: true`), and ~6–8s later every tool call in the still-running child fails with working directory no longer exists / Refusing to run there (#94221). Score ouster or admit tenanted. Folder: `projects/ouster/`. Still listed. Not featured.

**Listed: [Proscription](https://hermes-playground-green.vercel.app/proscription/)** — Roman outlaw-list / wax-tablet forum / iron-stylus / marble-lintel / torch-lit senate booth / Cinzel + Source Sans 3 + JetBrains Mono. The tablet should stay barred (deny list actually strips tools; chalked names absent from the forum); instead a custom subagent's own frontmatter `disallowedTools` does not strip tools — Bash still loaded and executes, WebSearch ToolSearch-found, MCP full name runs; only `project_delete` is denied by the auto mode classifier (#94202). Score proscription or admit barred. Folder: `projects/proscription/`. Still listed. Not featured.

**Listed: [Diabolica](https://hermes-playground-green.vercel.app/diabolica/)** — inquisitorial / devil's-proof / parchment-court / iron-balance-scale / sealed-writ / candlelit-chamber booth / Fraunces + Nunito Sans + Fira Code. The court should stay innocent (sealed writ shut; iron scale balances; six charges wax-shut); instead worktree-isolated sessions still refuse Bash commands that never run git (2.1.270) — residual inverted burden of proof (cannot be shown not to be git / too complex to verify) after loops and quoted-argv substring git often pass (#94040). Score diabolica or admit innocent. Folder: `projects/diabolica/`. Still listed. Not featured.

**Listed: [Sallyport](https://hermes-playground-green.vercel.app/sallyport/)** — fortress sallyport / gatehouse / iron-grille side-passage / torch-lit stone corridor / sealed-strongroom booth / Newsreader + Public Sans + Source Code Pro. The fortress should stay sealed (both doors shut; keys stay redacted; PreToolUse still sees Bash); instead a harness "file changed on disk" `<system-reminder>` dumps a secret file's full current contents past a locked PreToolUse gate because the injection is not a tool call (#94082). Score sallyport or admit sealed. Folder: `projects/sallyport/`. Still listed. Not featured.

**Listed: [Palilalia](https://hermes-playground-green.vercel.app/palilalia/)** — speech-pathology clinic / phonograph-groove / wax-cylinder / repeating-stylus booth / Libre Baskerville + DM Sans + JetBrains Mono. The groove should stay silenced (stylus lifts; /goal does not re-fire; hold is acknowledged); instead a session-scoped `/goal` Stop hook re-fires indefinitely with unchanged or stale text, even after verifiable evidence the condition is met or a deliberate hold, and only the repeated-block safety valve ends the loop (#94041). Score palilalia or admit silenced. Folder: `projects/palilalia/`. Still listed. Not featured.

**Listed: [Sepulchre](https://hermes-playground-green.vercel.app/sepulchre/)** — stone sepulchre / burial vault / sealed tomb / ossuary niche / limestone lintel / extinguished-lamp booth / Cardo + Figtree + IBM Plex Mono. The vault should stay living (lamp still burns; JSON stream intact; no NUL in the next request); instead a Bash tool result containing NUL characters (U+0000) truncates the next API request body, the server returns 400 unexpected end of data, and the session never recovers (#94055). Score sepulchre or admit living. Folder: `projects/sepulchre/`. Still listed. Not featured.

**Listed: [Sneck](https://hermes-playground-green.vercel.app/sneck/)** — Northern cottage / workshop-door / brass-sneck-latch / slate threshold / wool draft / iron latch plate / rain-on-the-stoop booth / Fraunces + Nunito Sans + Source Code Pro. The latch should stay cleared (Hide held; leaf stays undone; active file withheld from every prompt); instead VS Code 2.1.268 replaced Hide with an X whose dismissSelection is per-file, so a tab switch and return brings the chip back and there is no setting to stop auto-attach (#94052). Score sneck or admit cleared. Folder: `projects/sneck/`. Still listed. Not featured.

**Listed: [Drawbridge](https://hermes-playground-green.vercel.app/drawbridge/)** — medieval castle drawbridge / portcullis / bailey-approach / gatehouse / raised-span booth / merlons / ditch water / torch sconces / Cinzel + Manrope + JetBrains Mono. The approach should stay spanned (remote carts reach the keep; live sessions stay linked; gatehouse ledger matches live ids); instead a desktop Windows auto-update (2.1.266 → 2.1.270) raises the machine-wide Remote Control span, every unrelated bailey goes offline at once, halls stay lit, no horn sounds, and the ledger only admits future carts (#94049). Score drawbridge or admit spanned. Folder: `projects/drawbridge/`. Still listed. Not featured.

**Listed: [Chirograph](https://hermes-playground-green.vercel.app/chirograph/)** — medieval chirograph / bipartite-indenture / wavy-cut charter-desk / scriptorium lectern booth / two matching parchment moieties / wax seal / gall ink / Cormorant Garamond + Figtree + IBM Plex Mono. The indenture should stay matched (recorded branch == live branch; reclaim succeeds; uncommitted work preserved); instead `git branch -m` inside the worktree is never observed, recycle recovery dies `fatal: invalid reference`, and the session falls back to the shared main repo with uncommitted loss and a false-reassurance branch (#94045). Score chirograph or admit matched. Folder: `projects/chirograph/`. Still listed. Not featured.

**Listed: [Titulus](https://hermes-playground-green.vercel.app/titulus/)** — Roman inscription / marble name-plaque / funerary-titulus booth / veined marble / bronze lettering / terracotta recut / lapis lintel / Forum + Outfit + Space Mono. The plaque should stay inscribed (latest rename wins; phone name and desktop cache stay synced); instead an iOS rename reaches the transcript while the Windows Code sidebar and resume append keep the auto-generated title (#94025). Score titulus or admit inscribed. Folder: `projects/titulus/`. Still listed. Not featured.

**Listed: [Derelict](https://hermes-playground-green.vercel.app/derelict/)** — maritime abandoned-hulk / derelict-ship / salvage-yard booth / rust hull plates / fogbound pier / broken mast / bilge / lantern / tide marks / smoking cargo / Spectral + Nunito Sans + Fira Code. The pier should stay berthed (process-group teardown; session death kills the Bash-tool tree); instead a stopped/crashed/cleared session leaves tsc/vitest reparented to PID 1, unsupervised for hours (#93996). Score derelict or admit berthed. Folder: `projects/derelict/`. Still listed. Not featured.

**Listed: [Vestry](https://hermes-playground-green.vercel.app/vestry/)** — liturgical vestry / sacristy / peg-rail booth / cool stone / indigo stole / linen vestments / brass pegs / candle-smoke / Cormorant Upright + Karla + IBM Plex Mono. The rail should stay pegged (concurrent hangers respected; shared rail keeps others' mounts); instead Linux bwrap placeholder mount cleanup is per-process with no cross-process refcount so concurrent sessions on one project root delete each other's mounts (#94008). Score vestry or admit pegged. Folder: `projects/vestry/`. Still listed. Not featured.

**Listed: [Anarthria](https://hermes-playground-green.vercel.app/anarthria/)** — ENT / laryngology / voice-clinic booth / vertical voice-strip / laryngoscope chart / glottis rings / clipboard-to-larynx stack / Fraunces + Karla + JetBrains Mono. The larynx should stay articulate (paste lands; prompt receives dictation); instead Claude Code 2.1.269 silently drops Wispr Flow clipboard+Ctrl+V in the VS Code integrated terminal over Remote-WSL (#93782). Score anarthria or admit articulate. Folder: `projects/anarthria/`. Still listed. Not featured.

**Listed: [Trismus](https://hermes-playground-green.vercel.app/trismus/)** — oral-surgery / lockjaw / trigeminal clinic booth / enamel chair / forceps tray / nerve chart / clamped jaw on the completion chime / steel instruments / clinical tile / Archivo Black + Figtree + IBM Plex Mono. The chair should stay limber (main thread free; notification post/close never blocks UI; jaw opens); instead a Code-tab terminal-done chime deadlocks the main thread inside `swift_addon.node` UNUserNotification XPC (add vs close) (#93823). Score trismus or admit limber. Folder: `projects/trismus/`. Still listed. Not featured.
...
**Listed: [Scotoma](https://hermes-playground-green.vercel.app/scotoma/)** — ophthalmology / Humphrey-style visual-field / perimetry booth / exam ink / bowl slate / chart bone / fixation gold / blind-spot carmine / perimetry teal / Humphrey bowl / fixation cross / field grid / reliability FL·FP·FN / Libre Baskerville + Figtree + JetBrains Mono. The chart should stay legible (evaluator reads the /goal instruction; field clear); instead the goal lived only in `<command-args>` and Stop fired ~9 times then declared itself unachievable (#93744). Score scotoma or admit legible. Folder: `projects/scotoma/`. Still listed. Not featured.

**Listed: [Aneroid](https://hermes-playground-green.vercel.app/aneroid/)** — aneroid-barometer / meteorological instrument-panel booth / panel charcoal / dial brass / storm amber / phosphor cyan / sealed-glass teal / dual concentric dial / Settings chip / hover balloon / U≥50 suppression lamp / barograph strip / Orbitron + Exo 2 + IBM Plex Mono. The capsule should stay calibrated (ring scored against autoCompactWindow; warning with runway before compact); instead the webview scored the model's 1M window and suppressed the ring until ~500k so compact fired with no warning (#93901). Score aneroid or admit calibrated. Folder: `projects/aneroid/`. Still listed. Not featured.

**Listed: [Simulacrum](https://hermes-playground-green.vercel.app/simulacrum/)** — Baudrillard / hyperreality museum booth / wax-museum CRT mannequins / museum charcoal / wax-cream / phosphor green / cold violet void / brass plaque / dead-registration amber / glass case / process meter at zero / list-vs-switch rail / Syne + Outfit + IBM Plex Mono. The glass should stay tethered (real extension reachable; navigate drives a live window); instead list reported connected and navigate stamped Navigated with no browser process (#93751). Score simulacrum or admit tethered. Folder: `projects/simulacrum/`. Still listed. Not featured.

**Listed: [Solenoid](https://hermes-playground-green.vercel.app/solenoid/)** — industrial switchgear / solenoid-coil atelier / slate steel / copper coil / navy void / mint engaged / rust inert / coil-plunger / Settings vs settings.json rail / warm→focus→pty→gap timeline / Mac-absent vs Win-disconnected chips / Big Shoulders Display + Barlow + IBM Plex Mono. The coil should stay engaged (RC arms at warm/focus, before any message); instead the Settings toggle was inert and RC waited for first sendMessage (#93754). Score solenoid or admit engaged. Folder: `projects/solenoid/`. Still listed. Not featured.

**Listed: [Scotia](https://hermes-playground-green.vercel.app/scotia/)** — classical scotia / shadow-gap / column-molding booth / limestone / scotia hollow / DECSTBM brackets / VTE chip / blank-row void strip / Linux chip / XTVERSION / Cormorant Garamond + Outfit + IBM Plex Mono. The column should stay flush (bottom block on the last row; no hollow gap); instead DECSTBM left a 2–3 row scotia under the prompt on Linux VTE (#93764). Score scotia or admit flush. Folder: `projects/scotia/`. Still listed. Not featured.
...
**Listed: [Rasure](https://hermes-playground-green.vercel.app/rasure/)** — parchment rasure / scriptorium scraping booth / ghost parchment leaf / margin rules / wax seal chip / CreationTime stamp flip / blank `.claude.json` stub / secrets-folder badge / backup-timestamp chip / Crimson Pro + Manrope + Source Code Pro. The leaf should stay intact (CreationTime stable; config survives); instead `~/.claude` is wiped and recreated wholesale so CreationTime flips, stubs replace history and hooks, and incident 4 also took `secrets/` (#93791). Score rasure or admit intact. Folder: `projects/rasure/`. Still listed. Not featured.

**Listed: [Ashpan](https://hermes-playground-green.vercel.app/ashpan/)** — industrial grate / ashpan / foundry booth / iron grate bars / ash pan tray / ember glow / soot / ledger stamps / UUID mapping chips / spawned-child badge / Teko + Nunito Sans + Fira Code. The transcript should stay swept (gone with the index); instead delete_session burns the ledger for a spawned child while the CLI-uuid jsonl sits in the pan (~2.9MB, parseable) (#93780). Score ashpan or admit swept. Folder: `projects/ashpan/`. Still listed. Not featured.

**Listed: [Outrider](https://hermes-playground-green.vercel.app/outrider/)** — cavalry outrider / dispatch-rider booth / cavalry silhouette / sealed dispatch pouch / unfinished helpers racing / bare-header gate stamp / timeline chips / needs-auth branding iron / Archivo Black + Barlow + Share Tech Mono. The connect should stay credentialed until headersHelper returns; instead a bare connect rode ahead, 403 with no Authorization, valid token discarded 135ms later, gate stamped needs-auth (#93776). Score outrider or admit credentialed. Folder: `projects/outrider/`. Still listed. Not featured.

**Listed: [Necrology](https://hermes-playground-green.vercel.app/necrology/)** — parish necrology / death-register / sexton-desk booth / open folio / ruled columns / torn census strip / sexton ink pot / visitation ribbon / Cardo + Sora + JetBrains Mono. A still-living recent model should stay attested after a second witness; instead one incomplete `/models` listing (non-JSON, then retried) necrologized it as "does not exist" inside a blocking multiple-choice (#93774). Score necrology or admit attested. Folder: `projects/necrology/`. Still listed. Not featured.

**Listed: [Innominate](https://hermes-playground-green.vercel.app/innominate/)** — innominate nameplate / blank-escutcheon booth / blank nameplate / icon-only Send-Stop / focus-ring halo / live-region ticker / UIA Name meter / WCAG 4.1.2 plaque / Fraunces + Atkinson Hyperlegible + IBM Plex Mono. The Send/Stop plate should stay named; instead UIA Name is empty `""` in both states because children are icon-only and no aria-label is set (#93769). Score innominate or admit named. Folder: `projects/innominate/`. Still listed. Not featured.

**Listed: [Snuffer](https://hermes-playground-green.vercel.app/snuffer/)** — candle-snuffer / taper booth / artifact flame / scratchpad taper / brass snuffer cup / ganged-or bar / environment strip / one-way door / Playfair Display + Source Sans 3 + Fira Code. The scratchpad taper should stay lit when only artifacts are snuffed; instead `enableArtifact: false` also extinguishes `Scratchpad directory:` because eligibility wrongly ORs artifact eligibility after 2.1.186 (#93746). Score snuffer or admit lit. Folder: `projects/snuffer/`. Still listed. Not featured.

**Listed: [Changeling](https://hermes-playground-green.vercel.app/changeling/)** — fairy-court / cradle-swap booth / cradle / court ledger / swapped swaddling / remote latch / model identity token / invisible reinjection / Cinzel + Lexend + JetBrains Mono. A session pledged with `/model` should keep that heir; instead remote reconnect re-injects the global default so the lookalike runs while the ledger still names Opus (#93757). Score changeling or admit pledged. Folder: `projects/changeling/`. Still listed. Not featured.
...
**Listed: [Irons](https://hermes-playground-green.vercel.app/irons/)** — sailing in-irons / head-to-wind booth / helm locked into the wind / sails aback / scheduled chronometer / WebSearch kite that never fills on the cron path / interactive wind gauge / Cardo + Figtree + IBM Plex Mono. A scheduled/cron WebSearch should stay underway (identical interactive query returns in seconds); instead the kite never fills — no result, no error, no timeout — `lastActivityAt` freezes and the session stays running (#93615). Score irons or admit underway. Folder: `projects/irons/`. Still listed. Not featured.

**Listed: [Cathead](https://hermes-playground-green.vercel.app/cathead/)** — ship’s bow cathead / anchor-timber / PTY-slot booth / oak timber / iron ring and cat-block / slot-vector gauge / placeholder `cat` line / respawn lever / ENXIO flare / kernel log strip / Literata + Sora + Red Hat Mono. A macOS teammate pane should stay seated (cathead holds the new pane; no placeholder-cat + respawn-pane -k race; forkpty opens a live PTY); instead TmuxBackend coils `cat` then `respawn-pane -k` frees the last slot mid-open so grow is skipped and ENXIO drops the pane (#93624). Score cathead or admit seated. Folder: `projects/cathead/`. Still listed. Not featured.

**Listed: [Anachronism](https://hermes-playground-green.vercel.app/anachronism/)** — film continuity / slate chronometer booth / clapperboard timestamps / darkroom chronometer / sprocket rail / dual clocks (pre-warm 09:03 vs session 09:12) / reflog strip / Spectral + Figtree + IBM Plex Mono. A Claude Code on the web session should stay on tip (HEAD at remote tip as of session start; checkout -B / reset --hard onto FETCH_HEAD; SessionStart hooks see the new commit); instead the harness fetches the new tip then `git checkout` rolls the pre-warm local and rewrites `origin/<branch>` so status lies up to date (#93585). Score anachronism or admit tip. Folder: `projects/anachronism/`. Still listed. Not featured.
...
**Listed: [Disseisin](https://hermes-playground-green.vercel.app/disseisin/)** — court-of-novel-disseisin / freehold manor-roll booth for #93574. Still listed. Not featured.

**Listed: [Analepsis](https://hermes-playground-green.vercel.app/analepsis/)** — manuscript flashback / collation-desk booth / verso folio / recto gathering / splice gutter / dusk horizon / stuck quill / Libre Baskerville + Figtree + IBM Plex Mono. A Desktop Code-tab feed should stay ordered (disk, React tree, and bottom-anchored list agree; latest result at the bottom); instead a synthetic `background_tasks_redelivered` marker splices the 18:32–19:24 stretch after 22:30 so the reader lands on an hours-earlier turn and the working marker sticks (#93569). Score analepsis or admit ordered. Folder: `projects/analepsis/`. Still listed. Not featured.

**Listed: [Monstrance](https://hermes-playground-green.vercel.app/monstrance/)** — sanctuary monstrance / exposition booth / gilt rays / luna glass / altar step / phantom DENY ribbon / sacristy shelf / Gilda Display + Mulish + Anonymous Pro. Artifact should stay viewed (live artifact readable via the fetch Cowork actually provides); instead the read path binds withdrawn native WebFetch and veils the host behind a phantom deny (#93563). Score monstrance or admit viewed. Folder: `projects/monstrance/`. Still listed. Not featured.

**Listed: [Cipherlock](https://hermes-playground-green.vercel.app/cipherlock/)** — vault / bank-safe / cipher-lock booth / brass combination dial / steel door / keycard slot / Cinzel + Source Sans 3 + IBM Plex Mono. MCP OAuth tokens in the shared Keychain item `Claude Code-credentials` should stay sealed (valid access + refresh held); instead concurrent `claude` processes plus `claude mcp list` from another shell (one killed mid-run) rewrite notion/atlassian entries with empty `accessToken`/`refreshToken` and no `clientId` while the refresh tokens are still valid (#93537). Score cipherlock or admit sealed. Folder: `projects/cipherlock/`. Still listed. Not featured.

**Listed: [Attainder](https://hermes-playground-green.vercel.app/attainder/)** — parchment bill-of-attainder / court-of-attainder booth / wax seal / iron stamp / rolled parchment / clerk desk / Old Standard TT + Public Sans + IBM Plex Mono. A real user rejection of a shown prompt should stay untainted (`user-rejected` reserved for a genuine prompt "no"); instead parked-permission retirement after an internal session reset stamps `toolDenialKind:"user-rejected"` with no prompt and no keypress (#93529). Score attainder or admit untainted. Folder: `projects/attainder/`. Still listed. Not featured.
...
**Listed: [Pentimento](https://hermes-playground-green.vercel.app/pentimento/)** — art-conservation / underpainting atelier booth / gallery plaster / linseed umber / cobalt underpaint / varnish amber / charcoal stretcher / Fraunces + Source Sans 3 + IBM Plex Mono. After `device_commit_files` reports overwrite success the canvas should stay flushed (disk bytes == committed payload, mtime honest); instead the fresh varnish (mtime) covers a one-behind underpainting (previous commit's bytes) (#93482). Score pentimento or admit flushed. Folder: `projects/pentimento/`. Still listed. Not featured.

**Listed: [Vinculum](https://hermes-playground-green.vercel.app/vinculum/)** — chain-forge / binder's vinculum bench / iron slate / brass nlink gauges / linen binding cord / soot / Libre Baskerville + Karla + Source Code Pro. A workspace file should stay solitary (nlink=1, no Claude-owned alias, cloud file bridge accepts); instead local agent mode hardlinks into `%APPDATA%\Claude\local-agent-mode-sessions\...\uploads\` so nlink>1 and the cloud bridge refuses (#93485). Score vinculum or admit solitary. Folder: `projects/vinculum/`. Still listed. Not featured.
...
**Listed: [Fomite](https://hermes-playground-green.vercel.app/fomite/)** — sterile-lab / epidemiology booth / glass slide / culture dish / agar / pathogen droplet hitchhiking on a directory-marketplace install copy / sterile white / teal / agar amber / pathogen crimson / glass cyan / Alegreya + Nunito Sans + Fira Code. A directory marketplace `claude plugin install` should stay scrubbed (gitignore honored; no `.env` / `node_modules` in the versioned cache; install matches the tracked tree); instead the copy excludes `.git` but never reads `.gitignore`, so a root `.env` and 51M of `node_modules` hitchhike into a 128M cache against 4.5M tracked (#93423). Score fomite or admit scrubbed. Folder: `projects/fomite/`. Still listed. Not featured.

**Listed: [Snubber](https://hermes-playground-green.vercel.app/snubber/)** — hydraulic / pneumatic pulse-damper booth / snubber canister / copper line / pressure-gauge needle / srt-mux manifold / leaked peer valves / EPIPE pulse / bench iron / hydraulic amber / copper / pulse crimson / compressed air / Fraunces + Outfit + IBM Plex Mono. A sandboxed Bash command should stay damped (peer closes clean; mux listener only; no EPIPE spin); instead kill -9 mid-stream leaks the accepted `srt-mux` socket and the main-thread write loop busy-spins on EPIPE at 100%+ CPU (#93398). Score snubber or admit damped. Folder: `projects/snubber/`. Still listed. Not featured.

**Listed: [Fosse](https://hermes-playground-green.vercel.app/fosse/)** — earthwork / defensive-ditch booth / wet clay trench cut / sod lip / chalk survey marks / iron spike markers / mist over the fosse / host bank vs guest void / trench ink / wet clay / chalk / iron / sod / mist haze / Source Serif 4 + Karla + Roboto Mono. Cowork on Windows 10 Pro 22H2 should stay mounted (4/4 Plan9 shares under `/mnt/.virtiofs-root/shared`; host hr=0x0 AND guest mounts succeed); instead after the September 2026 cumulative the host still reports `HcsModifyComputeSystem` hr=0x0 while the guest mounts 0/4 with `Plan9 mount failed: invalid argument` (#93358). Score fosse or admit mounted. Folder: `projects/fosse/`. Still listed. Not featured.

**Listed: [Hibernacle](https://hermes-playground-green.vercel.app/hibernacle/)** — winter hibernacle / animal wintering-den booth / cold stone / frost linen / ice-blue breath / moss-green bedding / ember stall-meter / deep slate night / frost / ice / moss / ember / Cormorant Garamond + Sora + JetBrains Mono. Claude Code on Windows should stay warm (working set resident while idle; first Enter after idle paints immediately; no majflt storm); instead Windows trims the idle process with 12.5 GB RAM free, first Enter after ~2+ minutes blocks the Node event loop ~5s with majflt≈41105 / cpu≈77ms, the TUI does not repaint, and a second Enter double-submits (#93372). Score hibernacle or admit warm. Folder: `projects/hibernacle/`. Still listed. Not featured.

**Listed: [Scapegoat](https://hermes-playground-green.vercel.app/scapegoat/)** — desert ritual / scapegoat-altar booth / ash altar / ochre dust / bone linen / rust-blood mark / goat-bell / grant-table / desert ash / ochre / bone / rust-blood / goat-ink / dry-wind haze / Libre Bodoni + Outfit + IBM Plex Mono. Claude-in-Chrome should stay honest (check host grant before waiting for `document_idle`; return the same immediate named permission denial `computer` click already returns; name the host; never assert page-loading when injection was never established); instead an ungranted host makes executeScript hang 45000ms / 5000ms and blame a complete page (#93348). Score scapegoat or admit honest. Folder: `projects/scapegoat/`. Still listed. Not featured.

**Listed: [Cartulary](https://hermes-playground-green.vercel.app/cartulary/)** — monastic charter-register / cartulary-desk booth / oak lectern / bound quires of grants / register index / inkhorn / candle / oak / vellum cream / oak-gall ink / oxidized copper / candle amber / soot / Fraunces + Karla + IBM Plex Mono. `mcpOAuth` should stay bound (one record per connector; key by `mcp_server_id`; update in place; prune ended sessions; identical tokens deduped); instead every session re-stores identical claude.ai connector tokens under a new session-scoped `serverUrl` so `.credentials.json` accretes to 1,681 records / 906 KB, same token ×113 (#93331). Score cartulary or admit bound. Folder: `projects/cartulary/`. Still listed. Not featured.

**Listed: [Paraph](https://hermes-playground-green.vercel.app/paraph/)** — notarial / signature-paraph / issuer-seal booth / notarial instrument / wax press / issuer ribbon / signature flourish / notarial ink / parchment / wax-seal crimson / brass / graphite / Literata + Figtree + Roboto Mono. Desktop BYO OAuth MCP connect should keep issuer strings sealed (real quotes; formatter shows real values; OAuth window opens; Desktop matches CLI); instead the version-negotiation probe fails with a malformed RFC 8414 §3.3 Issuer mismatch whose quotes open real and close `%22` (#93327). Score paraph or admit sealed. Folder: `projects/paraph/`. Still listed. Not featured.

**Listed: [Appanage](https://hermes-playground-green.vercel.app/appanage/)** — royal-grant / heraldic inheritance desk / letters patent / cadency marks / wax cost-seal / lesser coronet versus parent crown / plum field / gold leaf / seal vermilion / vellum / purpure / Playfair Display + Source Sans 3 + IBM Plex Mono. Bundled `code-review` children should stay routed to an explicit cheaper model; instead a model-initiated fork lets every Agent child inherit the parent `claude-fable-5-1` crown with no cost gate and no policy reach (#93307). Score appanage or admit routed. Folder: `projects/appanage/`. Still listed. Not featured.

**Listed: [Pontoon](https://hermes-playground-green.vercel.app/pontoon/)** — harbor pontoon / floating-bridge pier / timber decking / salt fog / navigation lights / rope / tide marks / ink navy / wet timber / fog silver / nav-light amber / rope tan / Petrona + Figtree + Azeret Mono. Remote Control bridges should stay afloat after a Desktop restart; instead stealth relaunch / onQuitCleanup washes every session bridge while the sidebar still looks intact, and the phone list is empty (#93288). Score pontoon or admit afloat. Folder: `projects/pontoon/`. Still listed. Not featured.

**Listed: [Concordat](https://hermes-playground-green.vercel.app/concordat/)** — diplomatic chancery / treaty-desk / parchment / seal-wax / Vollkorn + DM Sans + Inconsolata. After SEP-2575 discover, Desktop and CLI send `Mcp-Protocol-Version: 2025-11-25` while `_meta` carries `2026-07-28`; a spec-compliant stateless server rejects every call (`-32020`) (#93290). Score concordat or admit concordant. Folder: `projects/concordat/`. Still listed. Not featured.

**Listed: [Revenant](https://hermes-playground-green.vercel.app/revenant/)** — Victorian séance parlor / process-tomb graveyard / charcoal bone / cold violet / rust commit-meter / candle soot / Young Serif + Mulish + DM Mono. Peer-session liveness probes should stay reaped (FFI or O(1); no wedged children); instead Windows falls back to Get-CimInstance Win32_Process with a 1s timeout, kills mid-RPC, and leaves unkillable ~43MB powershell revenants until reboot / commit exhaustion (#93274). Score revenant or admit reaped. Folder: `projects/revenant/`. Still listed. Not featured.

**Listed: [Replevin](https://hermes-playground-green.vercel.app/replevin/)** — legal replevin / writ desk / bond parchment / court green / bronze seal / ink black / Literata + Sora + Roboto Mono. ExitPlanMode should keep prePlanMode restored; instead iOS approval sends setMode auto and the rejection path falls back to default, displacing bypassPermissions (#93207). Score replevin or admit restored. Folder: `projects/replevin/`. Still listed. Not featured.

**Listed: [Cognate](https://hermes-playground-green.vercel.app/cognate/)** — comparative philology / manuscript cognate desk / parchment folios / ink indigo lemmas / ochre gloss marks / soft lamp / Fraunces + Source Sans 3 + IBM Plex Mono. Plugin MCP config should keep `${PLUGIN_ROOT}` expanded (spec-standard alias expands like `${CLAUDE_PLUGIN_ROOT}`; args/env/cwd resolve to the install path; session starts connected); instead the host only matches the Claude-prefixed dialect, leaves the standard cognate literal, and the MCP child gets `${PLUGIN_ROOT}/dist/index.js` verbatim → MODULE_NOT_FOUND / CONNECTION_CLOSED (#93250). Score cognate or admit expanded. Folder: `projects/cognate/`. Still listed. Not featured.

**Listed: [Lemures](https://hermes-playground-green.vercel.app/lemures/)** — Roman Lemuria / Parentalia night courtyard / black beans / bronze cymbals / chalk circles / bone-white masks / midnight indigo / charcoal / bronze / bone / Libre Baskerville + Red Hat Text + JetBrains Mono. The mid-turn task-summary classifier should stay laid across `/clear` (reset latestAsk / capturedIntent / taskSummary on conversation reset; multimodal prompts refresh latestAsk); instead process-scoped classifier state remanently keeps the previous conversation's latestAsk, image-bearing prompts never refresh it, emit stamps Date.now() so the staleness guard passes, and Remote Control mirrors the wrong task_summary (#93256). Score lemures or admit laid. Folder: `projects/lemures/`. Still listed. Not featured.

**Listed: [Escheat](https://hermes-playground-green.vercel.app/escheat/)** — feudal escheat chamber / royal escheator inquisition post mortem desk / twilight stone hall / parchment inquisitions / struck PID ledger / orphan worktree coffer / wax escheat seal / iron coffer / Cardo + Figtree + Fragment Mono. A Claude Code worktree session should keep the git worktree lock released when the session ends (shutdown that writes bridge-session / last-prompt also releases the lock; failing that, a later session treats a lock whose recorded PID is not running as stale and reaps it); instead VS Code window-close dies without releasing `.git/worktrees/<name>/locked`, the file keeps naming the dead PID, prune skips it, remove refuses without `--force`, and `--resume` from the main checkout does not list the session (#93231). Score escheat or admit released. Folder: `projects/escheat/`. Still listed. Not featured.

**Listed: [Mortmain](https://hermes-playground-green.vercel.app/mortmain/)** — medieval muniment-room / dead-hand charter booth / dark parchment / wax seals / iron chest drawers. Tracked `.claude` paths should stay freehold (git can check them out; tree matches HEAD; no phantom authorship); instead sandbox `denyWithinAllow` freezes writes under tracked `.claude/skills`, `.claude/hooks`, `.claude/settings.json` so switch leaves a half-updated tree that looks like deliberate WIP (#93173). Score mortmain or admit freehold. Folder: `projects/mortmain/`. Still listed. Not featured.
...
**Listed: [Procrustes](https://hermes-playground-green.vercel.app/procrustes/)** — blacksmith / innkeeper's iron bed bench / dark forge iron / linen sheets / rust-blood accents / brass measuring marks. An MCP server's full tool palette should stay intact while the server reports connected; instead one legal boolean property schema anywhere in one tool's `inputSchema` (e.g. `payload: true`, JSON Schema any-value) causes the Agent SDK CLI (0.1.77) converter to silently cull EVERY tool from that server — bisect 0/53 vs 53/53 — with zero diagnostics (#92900). Score culled or admit intact. Folder: `projects/procrustes/`. Still listed. Not featured.

**Listed: [Cadastre](https://hermes-playground-green.vercel.app/cadastre/)** — cadastral / land-registry surveyor's bench / green baize desk / brass theodolite / cream parcel maps / iron map-weights / inked roll. Claude Code should keep workspace-trust parcels enrolled on the `~/.claude.json` roll under a held stake; instead whole-file RMW writers abandon the mkdir lock after 15s, steal locks older than 10s, or write without it on mkdir failure — warn only — so `hasTrustDialogAccepted` parcels escheat and trust must be regranted (#92908). Score escheated or admit enrolled. Folder: `projects/cadastre/`. Still listed. Not featured.

**Listed: [Rubric](https://hermes-playground-green.vercel.app/rubric/)** — scriptorium / manuscript-rubricator bench / vermilion rubric numbers / cream parchment folio / oak desk / iron gall / candle. Claude Code TUI should echo the user's own ordered-list numbers as penned; instead when those numbers are not already sequential the TUI rewrites every item after the first so typed 3/2/1/4/5 displays as 3/4/5/6/7 and answers bind to the wrong questions — regression 2.1.234 (#92855). Score rewritten or admit as-penned. Folder: `projects/rubric/`. Still listed. Not featured.

**Listed: [Sheave](https://hermes-playground-green.vercel.app/sheave/)** — deck sheave / snatch-block bench / night-watch block station / hemp line through an iron wheel / brass pin / false hitch on a leading `/`. Mid-turn queue should pay any non-command string at the next tool boundary; a Finder-dragged `/Users/…` path fouls as a slash command and waits the whole turn (#92827). Score fouled or admit paid. Folder: `projects/sheave/`. Still listed. Not featured.

**Listed: [Mailslot](https://hermes-playground-green.vercel.app/mailslot/)** — brass postal mailslot / letter-flap bench / night post-office door / streetlamp / thick sealed packets vs narrow brass slot / counter stamp / Keychain vault. MCP OAuth via `claude mcp add` + `/mcp` login completes the browser exchange, but Keychain persist spills payloads over ~4KB at `security -i` stdin and the argv fallback dies with cryptic `pbt` while the UI stays unauthenticated (#92839). Score spilled or admit vaulted. Folder: `projects/mailslot/`. Still listed. Not featured.
...
**Listed: [Crenel](https://hermes-playground-green.vercel.app/crenel/)** — mason's battlement crenel / embrasure notch / limestone courses / chalk snap-lines / dusk battlement air. An MCP server advertising `capabilities.resources` as `{}` is treated as having none — ListMcpResourcesTool finds none, ReadMcpResourceTool says unsupported, tools/* still work (#92729). Score bricked or admit crenelled. Folder: `projects/crenel/`. Still listed. Not featured.

**Listed: [Quietus](https://hermes-playground-green.vercel.app/quietus/)** — registrar's quietus / death-knell ledger / dark ink / parchment / muted bronze bell / ledger ruling / quiet courtroom registry. A `SubagentStop` hook should fire when a background subagent is killed via `TaskStop` or `/exit` → "Exit and stop tasks", but the kill path clears the registry and stays unrung — normal completion still tolls (#92716). Score unrung or admit quieted. Folder: `projects/quietus/`. Still listed. Not featured.
...
**Listed: [Parison](https://hermes-playground-green.vercel.app/parison/)** — glasshouse / glory-hole parison bench. A hung gather that never reaches the punty is not a hold. Score the gather or admit marvered. Folder: `projects/parison/`. Still listed. Not featured.
...
**Listed: [Advowson](https://hermes-playground-green.vercel.app/advowson/)** — diocesan registry / patronage desk. A reserved living that silent-collates the built-in is not a hold. Score the presentation or admit vacant. Folder: `projects/advowson/`. Still listed. Not featured.

**Listed: [Smutch](https://hermes-playground-green.vercel.app/smutch/)** — binder's smutch bench. A home-folder Icon\r on every crate is not a hold. Score the smutches or admit plain. Folder: `projects/smutch/`. Still listed. Not featured.
...
**Listed: [Trammel](https://hermes-playground-green.vercel.app/trammel/)** — drafting trammel / split-sash desk. A hunting trammel is not a hold. Score the grooves or admit traced. Folder: `projects/trammel/`. Still listed. Not featured.
...
**Listed: [Davy](https://hermes-playground-green.vercel.app/davy/)** — miner's safety lamp / lamp station. A false canary is not a hold. Score the gauze or admit lit. Folder: `projects/davy/`. Still listed. Not featured.
...
**Listed: [Hydra](https://hermes-playground-green.vercel.app/hydra/)** — marble registry hydra desk. A settings cut that regrows from the known ledger is not a hold. Score both ledgers or admit cauterized. Folder: `projects/hydra/`. Still listed. Not featured.
...
`projects/clew/` is a rigger's / sailmaker's clew desk for Claude Code Bash sandbox deny lists that wind two filesystem deny entries per registered git worktree onto a single `/bin/bash -c "<bwrap …>"` argument until Linux `MAX_ARG_STRLEN` (128 KB). Then every Bash spawn dies with E2BIG — including `sleep 5` and the cleanup that would shrink the list (HEADLESS-BRICK / self-heal-none). A working-size coil is not a hold. Score the clew or admit **rove**. Verdicts: **rove**, **fouled**, **overcoiled**, **choked**, **twinned**, **swollen**, **jammed**, **pruned**, **cached**, **globbed**. Slack alarm on fouled / overcoiled / choked / jammed / swollen / cached / globbed. Linear ticket on fouled / choked / jammed. Sweep + `git worktree prune` without restart is **cached**, never **rove**.

Live path: https://hermes-playground-green.vercel.app/clew/

Demo mode needs no Slack / GitHub / Linear secrets and no npm. Seed `#90569` fouled is already on the loft. Idle word is **rove**, not the product name, not empty. NOT Wicket (Wicket scores whether isolation holds; Clew scores whether the CVE-mitigation deny list still lets `/bin/bash` spawn). NOT Scant (PATH truncation). NOT Sump (literal /dev/null LFS). NOT Cinch (silent partial mounts). NOT Hasp (file lease). NOT Sounder (missed background wakeup). NOT #51126 as a fix (closed not-planned: configure fewer user denies — #90569 has no user-configured deny rules). NOT leftover woodworking sliders. Different problem: deny list grows two entries per worktree → single bwrap argv crosses MAX_ARG_STRLEN → every Bash spawn dies with E2BIG. Different UI: sail loft. Hemp, tarred oak, brass thimble, lignum sheave, lantern, growing yarn ball toward a 128KB load line. Cinzel + Lora + Overpass Mono.
...
`projects/fusee/` is a clockmaker's fusee / conical-pulley desk for Claude Code scheduled tasks that dispatch ahead of their configured fire time. A written cron / fireAt is not a hold. Score the dial or admit **wound**. Verdicts: **wound**, **early**, **sprung**, **raced**, **ahead**, **jumped**, **premature**, **voided**, **held**, **true**. Slack fusee alarm on early / sprung / raced / ahead / jumped / premature / voided. Linear ticket on early / sprung / raced / ahead / premature.

Live path: https://hermes-playground-green.vercel.app/fusee/

Demo mode needs no Slack / GitHub / Linear secrets and no npm. Seed `#90485` early is already on the dial. Idle word is **wound**, not the product name, not empty. NOT Iota (path-key identity). NOT Leat (until-loop). NOT Shunt (nested SendMessage). NOT leftover woodworking sliders. Different problem: scheduler fires before configured fireAt/cron slot. Different UI: conical pulley, enamel dial, mainspring, arbor, winding key. Brass, lamp oil, oak case.
...
`projects/damper/` is a chimney damper / flue plate for Claude Code Remote Control that auto-enables without user consent. A settings toggle that reads off is not a hold. Score the draft or admit **banked**. Verdicts: **banked**, **drawn**, **vented**, **ajar**, **forced**, **defaulted**, **bridged**, **disclosed**, **sealed**, **lit**. Slack alarm on defaulted / drawn / forced / disclosed. Linear on defaulted / disclosed.
