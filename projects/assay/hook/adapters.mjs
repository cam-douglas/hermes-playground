/**
 * Assay sinks. Slack impurity alarm, GitHub assay ledger,
 * Linear incident on ghost or absorb (silent field loss).
 * Missing secrets stay honest: a demo row, never a fake live 200.
 */

function headline(result) {
  const session = result.session || "session";
  const issue = result.issue != null ? `#${result.issue}` : "";
  const tool = result.tool || "envelope";
  return `${session} · ${result.verdict}${issue ? ` · ${issue}` : ""} · ${tool}`;
}

function isQuiet(result) {
  const verdict = result.verdict || result.state;
  return verdict === "intact";
}

function isLinear(result) {
  return result.verdict === "ghost" || result.verdict === "absorb";
}

export function slackImpurityAlarm(result, env = process.env) {
  const webhook =
    env.ASSAY_SLACK_WEBHOOK || env.SLACK_WEBHOOK || env.SLACK_WEBHOOK_URL || "";
  const quiet = isQuiet(result);

  const text = quiet
    ? `Assay: cupel is ${result.verdict || "intact"} on ${result.session || "session"}.`
    : `Assay ${String(result.verdict || "").toUpperCase()} · ${headline(result)}`;

  const body = {
    text,
    blocks: [
      {
        type: "header",
        text: {
          type: "plain_text",
          text: quiet ? `Assay · ${result.verdict}` : `Assay · ${result.verdict} impurity alarm`,
        },
      },
      {
        type: "section",
        text: {
          type: "mrkdwn",
          text: [
            `*session* \`${result.session || "session"}\``,
            `*verdict* ${result.verdict}`,
            `*tool* \`${result.tool || "—"}\``,
            `*impurity* ${result.impurity || "none"}`,
            `*source* ${result.source || "—"}`,
            `*parseOk* ${result.parseOk}`,
            headline(result),
          ].join("\n"),
        },
      },
    ],
  };

  if (quiet) {
    return {
      adapter: "slack",
      mode: "demo",
      ok: true,
      summary: `Would skip Slack — cupel is ${result.verdict || "intact"}.`,
      body,
    };
  }

  if (!webhook) {
    return {
      adapter: "slack",
      mode: "demo",
      ok: true,
      summary: `Would post to Slack: impurity alarm — ${result.verdict} in the cupel.`,
      body,
    };
  }

  return {
    adapter: "slack",
    mode: "live",
    ok: null,
    summary: `Posting ${result.verdict} impurity alarm to Slack webhook.`,
    endpoint: webhook,
    body,
  };
}

export function githubAssayLedger(result, env = process.env) {
  const token = env.ASSAY_GITHUB_TOKEN || env.GITHUB_TOKEN || "";
  const line = JSON.stringify({
    at: new Date().toISOString(),
    product: "assay",
    session: result.session,
    verdict: result.verdict,
    alarm: result.alarm,
    source: result.source,
    issue: result.issue ?? null,
    action: result.action,
    tool: result.tool,
    parseOk: result.parseOk,
    retryFailed: result.retryFailed,
    contaminates: result.contaminates,
    impurity: result.impurity,
    missing: result.missing,
    residue: result.residue,
    admitted: result.admitted,
    refused: result.refused,
  });

  if (!token) {
    return {
      adapter: "github",
      mode: "demo",
      ok: true,
      summary: "Would append a GitHub assay ledger row. Demo: no token.",
      line,
    };
  }

  return {
    adapter: "github",
    mode: "live",
    ok: null,
    summary: "Would append a GitHub assay ledger row as a private gist.",
    tokenPresent: true,
    line,
  };
}

export function linearImpurityIncident(result, env = process.env) {
  const key = env.ASSAY_LINEAR_KEY || env.LINEAR_API_KEY || "";
  const loss = isLinear(result);
  const title = `Silent field ${result.verdict} · Assay · ${result.source || "envelope"}`.trim();
  const description = [
    "Assay refused a tool call because a parsed envelope is not a hold.",
    "",
    headline(result),
    "",
    result.verdict === "ghost"
      ? "Parse succeeded. A delivered string contains an adjacent parameter's boundary tag."
      : "A declared field vanished. Residue of its parameter grammar sits in a host field.",
    "",
    "Evidence (do not invent more): anthropics/claude-code#84405 #84362 #64774 #49747 #63879 #70544 #69522 #62123 #70657; openai/codex#19765 #31517 #26379.",
  ].join("\n");

  if (!loss) {
    return {
      adapter: "linear",
      mode: "demo",
      ok: true,
      summary: `Would skip Linear — cupel is ${result.verdict || "intact"}.`,
      title,
      description,
    };
  }

  if (!key) {
    return {
      adapter: "linear",
      mode: "demo",
      ok: true,
      summary: `Would open a Linear impurity incident: ${title}`,
      title,
      description,
    };
  }

  return {
    adapter: "linear",
    mode: "live",
    ok: null,
    summary: `Opening Linear impurity incident: ${title}`,
    title,
    description,
  };
}

export async function fire(result, env = process.env, fetchImpl = globalThis.fetch) {
  const planned = [
    slackImpurityAlarm(result, env),
    githubAssayLedger(result, env),
    linearImpurityIncident(result, env),
  ];
  const events = [];

  for (const plan of planned) {
    if (plan.mode !== "live" || !fetchImpl) {
      events.push({ ...plan, at: Date.now(), ok: true });
      continue;
    }

    try {
      if (plan.adapter === "slack" && plan.endpoint) {
        const response = await fetchImpl(plan.endpoint, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(plan.body),
        });
        events.push({
          ...plan,
          at: Date.now(),
          ok: response.ok,
          summary: response.ok
            ? `Posted ${result.verdict} impurity alarm to Slack.`
            : `Slack webhook failed: HTTP ${response.status}`,
        });
        continue;
      }

      if (plan.adapter === "github") {
        const token = env.ASSAY_GITHUB_TOKEN || env.GITHUB_TOKEN;
        const response = await fetchImpl("https://api.github.com/gists", {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/vnd.github+json",
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            description: `Assay ledger ${result.session || ""}`.trim(),
            public: false,
            files: { "assay-ledger.jsonl": { content: `${plan.line}\n` } },
          }),
        });
        const data = await response.json().catch(() => ({}));
        events.push({
          ...plan,
          at: Date.now(),
          ok: response.ok,
          summary: response.ok
            ? `Wrote private gist assay ledger ${data.id || ""}`.trim()
            : `GitHub gist failed: ${data.message || response.status}`,
        });
        continue;
      }

      if (plan.adapter === "linear") {
        const key = env.ASSAY_LINEAR_KEY || env.LINEAR_API_KEY;
        const teamId = env.ASSAY_LINEAR_TEAM || "";
        const response = await fetchImpl("https://api.linear.app/graphql", {
          method: "POST",
          headers: { Authorization: key, "Content-Type": "application/json" },
          body: JSON.stringify({
            query:
              "mutation IssueCreate($input: IssueCreateInput!) { issueCreate(input: $input) { success issue { id identifier url } } }",
            variables: {
              input: {
                title: plan.title,
                description: plan.description,
                ...(teamId ? { teamId } : {}),
              },
            },
          }),
        });
        const data = await response.json().catch(() => ({}));
        const issue = data?.data?.issueCreate?.issue;
        events.push({
          ...plan,
          at: Date.now(),
          ok: Boolean(issue),
          summary: issue
            ? `Opened Linear ${issue.identifier} ${issue.url}`
            : `Linear issueCreate failed: ${JSON.stringify(data.errors || data)}`,
        });
        continue;
      }

      events.push({ ...plan, at: Date.now(), ok: true });
    } catch (error) {
      events.push({
        ...plan,
        at: Date.now(),
        ok: false,
        summary: `${plan.adapter} failed: ${error instanceof Error ? error.message : "unknown"}`,
      });
    }
  }

  return { events };
}
