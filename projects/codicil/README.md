...
A **probate / will-chamber / codicil desk** atelier — attested will parchment, wax-seal lamp, clause strip showing Agent A's intended amend vs Agent B's usurped legatee, HEAD SHA before/after strip; ink-on-vellum / wax-seal crimson / parchment cream / clerk navy — Cormorant Garamond + Figtree + Azeret Mono — for a real Claude Code defect: **SHARED MULTI-AGENT WORKTREE — `git commit --amend` DOES NOT RE-CHECK HEAD; SILENTLY REWRITES CONCURRENT TEAMMATE COMMIT MESSAGE; AREA:AGENTS.**
...
Idle word: **attested**. Seeded state: **usurped** / #91513 — Agent A creates C1; before A's follow-up amend, Agent B commits C2 on top of C1; A's `git commit --amend` does not re-check HEAD and rewrites C2 (tree byte-identical; B's message discarded). Never idle as sealed / rewritten / swaged / torn / homed / crossed / armed / unheard.

A **codicil** should attest the deed on the desk: before `git commit --amend` in a possibly-shared tree, verify `git rev-parse HEAD` still equals the SHA from the agent's own prior commit, and refuse or warn if HEAD moved. `--amend` instead operates on whatever the current HEAD is, not on "the commit I personally just made."

- **usurped** = #91513: shared multi-agent worktree; `git commit --amend` does not re-check HEAD; silently rewrites concurrent teammate C2
...
- **no-rev-parse-guard** = `--amend` does not re-check `git rev-parse HEAD` against the agent's own prior SHA
...
- **attested** = HOLD: HEAD still agent's own SHA; amend safe

Verdicts: attested, usurped, head-moved, teammate-rewrite, message-discard, tree-identical, no-rev-parse-guard, shared-worktree, has-clear-repro, hold.

This is a diagnostic scoring desk. Not an exploit. No payloads. Score whether the shared-tree amend is attested or usurped.

Hypothesis only (NON-BINDING): product-level guardrail should refuse amend when HEAD ≠ agent's last commit SHA in shared trees; discard if issue evidence disagrees. Encoded from the issue's filed claim (A-C1 then B-C2 then A-amend; tree kept; message discarded). Do not claim Claude Code source you have not seen.
...
Different UI: probate / will-chamber / attested will parchment + wax-seal lamp + Agent A vs Agent B clause strip + HEAD SHA before/after strip / ink-on-vellum / wax-seal crimson / parchment cream / clerk navy. Cormorant Garamond + Figtree + Azeret Mono. NOT Literata/Fragment Mono. NOT Spectral/Public Sans/JetBrains Mono (Crimp). NOT Brygada/Atkinson/DM Mono (Jackfield). NOT Fraunces/Source Sans 3/IBM Plex (Tocsin). Stay OFF crimp pliers/foil / jackfield channel-strip / tocsin watchhouse / bolter flour-mill / deadeye standing-rigging / reglet letterpress / vault-latch / annunciator lamps.

Different verbs: Attest the deed, pin idle attested, pin seeded usurped, admit the HEAD already moved, load fixtures, reset to attested. Score the attestation is this desk's phrase.

Different idle: **attested**.
...
1. Idle demo loads **attested** — `git rev-parse HEAD` still equals Agent A's own C1 SHA; amend is safe.
2. Seed **usurped** → #91513: shared multi-agent worktree; `git commit --amend` does not re-check HEAD; Agent A C1 then Agent B C2 then A amends C2; C2 tree byte-identical; B's message discarded and replaced with A's.
3. Atelier UI: attested will parchment / wax-seal lamp / Agent A vs Agent B clause strip / HEAD SHA before/after strip. Attested = amend safe. Usurped = silent rewrite of a teammate's will. Wax-seal lamp lit on alarm.
...
5. **Attest the deed** walks the probe ticket and lights chips on the desk. Chip-switch every verdict. Paste or drop JSON.
...
Empty stdin scores the idle **attested** ticket. Paste a probe on the page or drop a fixture from `data/`.
