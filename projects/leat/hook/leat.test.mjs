...
test("50 catalog wiring: 34 products, Leat featured, Shunt and Sump listed", () => {
  ...
  assert.equal(featured[0].slug, "leat");
  assert.equal(featured[0].href, "/leat/");
  assert.equal(featured[0].day, "2026-08-29");
  assert.match(featured[0].summary, /08:50|blocked sleep is not a hold|stilled/);
...
test("51 vercel rewrite order puts /leat before /shunt, /sump and the slug fallback", () => {
...
test("52 hours.json prepends the 08:50 Sydney Leat ship", () => {
  ...
  assert.equal(hours[0].time, "08:50");
  assert.equal(hours[0].tz, "Australia/Sydney");
  assert.equal(hours[0].title, "Leat");
  assert.equal(hours[0].kind, "ship");
  assert.match(hours[0].note, /stilled/);
  assert.match(hours[0].note, /Shunt/);
  assert.match(hours[0].note, /Sump/);
  assert.match(hours[0].note, /34/);
