...
test("51 catalog wiring: 30 products, Scant featured, Chad listed", () => {
  ...
  assert.equal(featured[0].name, "Scant");
  assert.equal(featured[0].slug, "scant");
  assert.equal(featured[0].href, "/scant/");
  assert.equal(featured[0].day, "2026-08-29");
  assert.match(featured[0].summary, /04:50|scantling|written shell snapshot is not a hold|fit/);
...
test("52 vercel rewrite order puts /scant before /chad and the slug fallback", () => {
  ...
  assert.equal(sources[0], "/scant");
  assert.equal(sources[1], "/scant/");
